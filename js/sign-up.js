...
document
	.getElementById("loginUpWinterAdventures")
...
function settingDetails(inputNameUp, inputpasswordUp) {
	...
	if (!inputNameUp || !inputpasswordUp) {
		...
		//Conditional
		if (
			userDetailsArrayWinterAdventures === null ||
			userDetailsArrayWinterAdventures === undefined
		) {
			localStorage.setItem(
				"userDetailsArrayWinterAdventures",
			...
			userDetailsArrayWinterAdventures.push({
				name: inputNameUp,
				password: inputpasswordUp,
			});
			//with the push method I introduced our data in our array

			localStorage.setItem(
				"userDetailsArrayWinterAdventures",
				JSON.stringify(userDetailsArrayWinterAdventures)
			);
